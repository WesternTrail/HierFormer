...
class OverlapPatchEmbed(nn.Module):
    ...
    def _init_weights(self, m):  # 使用的是Xavier初始化
...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=3, num_classes=2, embed_dims=[32, 64, 128, 256],
        ...
        self.block1 = nn.ModuleList([LGLBlock(dim=embed_dims[0], num_heads=4, sr_ratio=sr_ratio[0], drop_path=dpr[i])
        ...
        self.block2 = nn.ModuleList(
            [LGLBlock(dim=embed_dims[1], num_heads=4, sr_ratio=sr_ratio[1], drop_path=dpr[i + depths[0]])
        ...
        self.block3 = nn.ModuleList(
            [LGLBlock(dim=embed_dims[2], num_heads=8, sr_ratio=sr_ratio[2], drop_path=dpr[i + depths[0] + depths[1]])
        ...
        self.block4 = nn.ModuleList([LGLBlock(dim=embed_dims[3], num_heads=8, sr_ratio=sr_ratio[3],
    ...
    def forward_features(self, feats):  # feats: 2B,3,256,256
        ...
        # feats = feats.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(feats)  # [(2B,64,H/4,W/4),(2B,128,H/8,W/8),(2B,320,H/16,W/16),(2B,512,H/32,W/32]
...
class FeatureFusionDecoder(nn.Module):
    ...
    def __init__(self, align_corners=True, in_channels=[64, 128, 320, 512], embedding_dim=256, output_nc=2,
        ...
        self.diff1 = Feature_Interaction_Module(in_channels=self.embedding_dim, height=2)
        self.diff2 = Feature_Interaction_Module(in_channels=self.embedding_dim, height=2)
        self.diff3 = Feature_Interaction_Module(in_channels=self.embedding_dim, height=2)
        self.diff4 = Feature_Interaction_Module(in_channels=self.embedding_dim, height=2)
    ...
    def forward(self, x_1, x_2):
        ...
        outputs = []
        # Stage 4: x1/32 scale
        _c4_1 = self.linear_c4(c4_1).permute(0, 2, 1).reshape(n, -1, c4_1.shape[2], c4_1.shape[3]) # 将c1,c2,c3,c4全部降低到同一个维度256
        _c4_2 = self.linear_c4(c4_2).permute(0, 2, 1).reshape(n, -1, c4_2.shape[2], c4_2.shape[3])
        _c4 = self.diff1([_c4_1, _c4_2])
        ...
        _c1 = self.diff4([_c1_1, _c1_2])
        p_c1 = self.make_pred_c1(_c1)
        outputs.append(p_c1)

        # Linear Fusion of difference image from all scales
        _c = self.linear_fuse(torch.cat([_c4_up, _c3_up, _c2_up, _c1], dim=1)) # b，1024，64，64 -> b,256,64,64
